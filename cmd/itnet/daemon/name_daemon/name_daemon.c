 ...
 ****************************************************************
 */
const char	pgversion[] =  "Versão:	4.4.0, de 09.04.03";
...
main (int argc, const char *argv[])
{
	 ...
	 *
	 *	Sintaxe:
	 *		name_daemon [-v] <name_server_addr> ...
	 */
	 ...
	 */
	for (index = 1, sp = servtb; *argv != NOSTR; argv++)
	{
		IPADDR		serv_addr;

		if ((serv_addr = t_str_to_addr (*argv)) == -1)
			{ error ("Endereço de servidor de nomes inválido: \"%s\"", *argv); continue; }

		if (sp >= &servtb[N_SERVTB])
			{ error ("Número excessivo de servidores de nomes"); break; }
