 ...
 ****************************************************************
 */
const char	pgversion[] =  "Versão:	4.8.0, de 08.09.05";
...
typedef struct
{
	...
	char	*s_serv_opt;	/* Opção para servidor */
...
#define N_NMSERVTB	10	/* No. máximo de servidores */

entry IPADDR	nmservtb[N_NMSERVTB];	/* A tabela de servidores de nomes */
...
read_itnet_serv (void)
{
	...
	STAT		s;
	char		new_state;
	char		*prot_nm, *port_nm, *serv_nm, *serv_opt;
	const char	*str;
	int		lineno = 0, port_no;
	char		buf[128];
	 ...
	 *	Le e processa o arquivo
	 */
	while (fngets (buf, sizeof (buf), fp) == buf)
	{
		lineno++;
		...
		new_state = *strfield (buf,   ':');
		prot_nm   =  strtok (strfield (NOSTR, ':'), "\t ");
		port_nm   =  strtok (strfield (NOSTR, ':'), "\t ");
		serv_nm   =  strtok (strfield (NOSTR, ':'), "\t ");
		serv_opt  =  strtok (strfield (NOSTR, ':'), "\t ");
#ifdef	DEBUG
		if (vflag) printf
		(	"state = '%c', prot_nm = \"%s\", port_addr = \"%s\", "
			"serv_nm = \"%s\", serv_opt = \"%s\"\n",
			new_state, prot_nm, port_nm, serv_nm, serv_opt
		);
#endif	DEBUG
		/*
		 *	Examina o protocolo
		 ...
		 *	Examina o "port" dado
		 */
		if ((port_no = strtol (port_nm, &str, 0)) < 1 || *str != '\0')
		{
			error
			(	"%s: No de \"port\" inválido: \"%s\"",
				edit_serv_file_msg (lineno), port_nm
			);
			exit_code++; continue;
		}
	   ...
	   /***	sp->s_old_state = ' ';	***/
		...
		/*
		 *	Examina a opção servidor
		 */
		if   (serv_opt == NOSTR || serv_opt[0] == '\0')
			sp->s_serv_opt = NOSTR;
		elif ((sp->s_serv_opt = strdup (serv_opt)) == NOSTR)
			out_of_memory ();
...
exec_nm_serv (void)
{
	int		i;
	char		**ap;
	char		*argv[N_NMSERVTB + 5];

	/*
	 *	Em primeiro lugar, verifica se deve ter servidor de nomes
	 */
	if (nmservtb_index == 0)
		return;
	...
	*ap++ = DOMAIN_NAME_SERVER_NM;
	...
	for (i = 0; i < nmservtb_index; i++)
	{
		*ap = alloca (16);
		strcpy (*ap++, t_addr_to_str (nmservtb[i]));
	}
...
re_read_itnet_serv (void)
{
	...
	while (fngets (buf, sizeof (buf), fp) == buf)
	{
		...
		if   (serv_opt == NOSTR || serv_opt[0] == '\0')
			sp->s_serv_opt = NOSTR;
		elif (!streq (sp->s_serv_opt, serv_opt) && (sp->s_serv_opt = strdup (serv_opt)) == NOSTR)
			out_of_memory ();
...
listen_fork (SERV *sp)
{
	 ...
	 *			    "tcp"
	 */
	sprintf (well_known_port_nm, "%d", sp->s_port_no);
