...
entry char		subject[80];	/* Assunto da carta */

entry const char	*header_file_nm; /* Nome do arquivo de cabeçalho */
...
#undef	DEBUG
	 ...
	 */
	if ((send_mail_file_nm = alloc_and_copy (tmpnam (NOSTR))) == NOSTR)
	...
	if   (mailrc_domain != NOSTR)
		fprintf (send_mail_fp, "%s@%s\r\n", local_user_nm, mailrc_domain);
	elif (from_with_host)
		fprintf (send_mail_fp, "%s@%s\r\n", local_user_nm, my_name);
	else
		fprintf (send_mail_fp, "%s@%s\r\n", local_user_nm, my_domain);

	for (dp = dst_head.d_forw; dp != &dst_head; dp = dp->d_forw)
		fprintf (send_mail_fp, "%s\r\n", dp->d_nm);
...
create_text_file (void)
{
	 ...
	 *	Obtém o arquivo
	 */
	if ((text_nm = alloc_and_copy (tmpnam (NOSTR))) == NOSTR)
		error ("$NÃO consegui obter o nome de um arquivo temporário");
...
quit (int code)
{
	if (header_file_nm != NOSTR)
		unlink (header_file_nm);
