...
const char	pgversion[] =  "Versão: 4.4.0, de 27.12.02";
...
int		examine_dst_nm (const DST *, const char *, CNTL *, int);
...
{ 
	...
	if (argc == 0)
	{
		reply (501, "Faltando o remetente do comando RCPT");
		return;
	}
	...
	{
		reply (501, "Não foi encontrado o destinatário do comando RCPT");
		return;
	}

	cntl.c_good_msg_no = 0;
	cntl.c_bad_msg_no  = 0;

	examine_dst_nm (NODST, dst_nm, &cntl, 0 /* Not from ".forward" */);

	if (cntl.c_bad_msg_no)
	{
		reply
		(	550, "<%s> ... destinatário com problemas",
			dst_nm
		);
	}
	else
 ...
 */
int
examine_dst_nm (const DST *orig_dp, const char *dst_nm, CNTL *cntl, int forward)
{
	DST			*dp;
	char			*user_nm;
	const char		*nm, *domain;
	 ...
	 *		user@node.domain
	 */
	if ((nm = strchr (dst_nm, '@')) != NOSTR)
		{ n = nm - dst_nm; nm++; }
	else
		n = strlen (dst_nm);

	user_nm = alloca (n + 1); strncpy (user_nm, dst_nm, n); user_nm[n] = '\0';
	...
	if (nm != NOSTR)					/* Tem "@", */
	{
		if ((domain = strchr (nm, '.')) != NOSTR)	/* e tem "." */
		{
			if (!streq (nm, my_node_nm) && !streq (nm, my_domain_nm))
			{
				if (forward || !streq (domain + 1, my_domain_nm))
					local = 0;
			}
		}
		elif						/* mas não tem "." */
		(	strlen (nm) != my_node_nm_len ||
			strncmp (nm, my_node_nm, my_node_nm_len)
		)
		{
				local = 0;
		}
	...
	{
		reply
		(	550|MORE, "<%s> ... destinatário desconhecido",
			dst_nm
		);
	...
	{
		reply
		(	550|MORE, "<%s> ... destinatário em \".forward\" cíclico",
			dst_nm
		);
   ...
   /***	dp->d_remote = 0; ***/
	...
	/*
	 *	É um "forward" para um computador remoto
	 */
    remote_dst:
...
{ 
	 ...
	 */
	if ((text_tmp_nm = strdup (tmpnam (NOSTR))) == NOSTR)
	{
...
send_to_remote_dst (void)
{
	 ...
	 *		Subject: Assunto
	 */
	if ((header_tmp_nm = strdup (tmpnam (NOSTR))) == NOSTR)
