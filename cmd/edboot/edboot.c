 ...
 ****************************************************************
 */
const char	pgversion[] =  "Versão:	4.6.0, de 23.10.04";
...
void		ed_boot2 (const char *, const char *, const char *);
...
main (int argc, const char *argv[])
{
	 ...
	 *	Analisa as opções
	 */
	while ((opt = getopt (argc, argv, "012wi:d:p:fvH")) != EOF)
	{
		switch (opt)
		{
	...
	switch (boot_code)
	{
		...
		ed_boot2 (file_nm, dev_str, pgm_str);
		break;
...
ed_boot0 (const char *file_nm, const char *index_str)
{
	 ...
	 *	Verifica se tem a assinatura "TROPIX BOOT0"
	 */
	if (!streq (file_ptr + BLSZ, "TROPIX BOOT0"))
 ...
 */
void
ed_boot2 (const char *file_nm, const char *dev_str, const char *pgm_str)
{
	NLIST		nl[4];
	HEADER		h;
	 ...
	 */
	if (dev_str == NOSTR && pgm_str == NOSTR)
	...
	lseek
	(	fd,
		sizeof (HEADER) + h.h_tsize + nl[1].n_value - h.h_dstart,
		SEEK_SET
	);
...
pr_boot2 (const char *file_nm, int fd, HEADER *hp, NLIST *nl)
{
	...
	lseek
	(	fd,
		sizeof (HEADER) + hp->h_tsize + nl[0].n_value - hp->h_dstart,
		SEEK_SET
	);
	...
	lseek
	(	fd,
		sizeof (HEADER) + hp->h_tsize + nl[1].n_value - hp->h_dstart,
		SEEK_SET
	);
	...
	lseek
	(	fd,
		sizeof (HEADER) + hp->h_tsize + nl[2].n_value - hp->h_dstart,
		SEEK_SET
	);
