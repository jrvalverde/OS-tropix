...
#define	MAX_TOTAL_FAT_12_BITS	(1 + 2 * 12 + 32 + (0xFF1 - 2))

/* No. máximo de CLUSTERs de um sistema com FAT de 16 bits */
 
#define	MAX_CLUSTER_FAT_16_BITS	(0xFFF1 - 2)

/* No. máximo do início (DOSSB, FAT e ROOT) de um sistema com FAT de 16 bits */
 
#define	MAX_PRO_FAT_16_BITS	(1 + 2 * 256 + 32)

/* Começo da FAT1 com FAT de 32 bits */
 
#define FAT_32_OFFSET		49

/* Duplicata do boot com FAT de 32 bits */
 
#define FAT_32_SB_BACKUP	6

/* Bloco de informação  da FAT de 32 bits */
 
#define FAT_32_SB_INFO		1

/* Número do CLUSTER da RAIZ FAT de 32 bits */
 
#define FAT_32_ROOT_CLUSTER	2
...
mk_one_fs (const char *dev_str, daddr_t sz, int cluster_sz)
{
	...
	if   (fat_bits == 32)
	{
		/* Estimativa acima do necessário */

		sec_fat = (dd.d_size / dd.d_sec_clus + 2) / (BLSZ / sizeof (long)) + 1;

		for (EVER)
		{
			n_clus  = dd.d_size;
			n_clus -= FAT_32_OFFSET + (N_FATs * sec_fat);
			n_clus /= dd.d_sec_clus;

			not_used = sec_fat * BLSZ - (n_clus + 2) * sizeof (long);
#undef	DEBUG
#ifdef	DEBUG
	...
	elif (fat_bits == 16)
	{
		...
		sec_fat = (dd.d_size / dd.d_sec_clus + 2) / (BLSZ / sizeof (short)) + 1;
		...
		for (EVER)
		{
			...
			not_used = sec_fat * BLSZ - (n_clus + 2) * sizeof (short);
#ifdef	DEBUG
			printf ("sec_fat = %d, not_used = %d\n", sec_fat, not_used);
#endif	DEBUG
			if   (not_used < 0)
...
#if (0)	/*******************************************************/
	   	bwrite (FAT_32_SB_INFO + 1, (void *)&fsinfo + BLSZ);
#endif	/*******************************************************/
	}
