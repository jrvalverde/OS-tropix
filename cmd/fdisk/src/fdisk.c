 ...
 ****************************************************************
 */
const char	pgversion[] =  "Versão: 4.6.0, de 11.09.04";
...
int		fdisk_read_parttb (const char *block0, int fd);
...
void		fdisk_get_cmd (char *, int fd, char *block0);
...
main (int argc, const char *argv[])
{
	...
	STAT		s;
	DISKTB		dt;
	char		block0[BLSZ];	/* Para o bloco 0 do disco */
	...
	if (gflag || gp->h_head == 0 || gp->h_sect == 0)
		geo_get_parttb_geo (block0, gp, fdisk_size, file_nm, 1);

	fdisk_hdinfo = gp;

	fdisk_round = gp->h_head * gp->h_sect;

	if (fdisk_read_parttb (block0, fd) < 0)
		return (1);

	fdisk_get_cmd ("", fd, block0);
 ...
 */
void
fdisk_get_cmd (char *cmd, int fd, char *block0)
{
	char		*cp;
	char		line[80];
	PARTTB		*pp;
	...
	switch (*cp)
	{
		 ...
		 */
	    case 'g':
		if (geo_get_parttb_geo (block0, &hi, 0, file_nm, 0) < 0)
...
#endif	DEBUG
		llseek (fd, (loff_t)0, NULL, L_SET);

		if (write (fd, block0, BLSZ) != BLSZ)
 ...
 */
int
fdisk_read_parttb (const char *block0, int fd)
{
