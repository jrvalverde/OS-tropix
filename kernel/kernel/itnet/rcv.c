 ...
 */
void
receive_tcp_packet (register ITBLOCK *bp, void *area, int count)
{
	register ITSCB	*ip = &itscb;
	register TT_H	*thp = (TT_H *)(area - sizeof (PSD_H));
	register TCP_EP	*tp;
	register char	OK = 0;
	register ulong	seg_seq;
	register ulong	rcv_wnd;
	register ulong	rcv_nxt;
	register int	n;
	 ...
	 */
	if
	(	(bp->it_count = count - h_size) < 0 ||
		compute_checksum (thp, sizeof (PSD_H) + count) != 0
	)
	{
#ifdef	MSG
   ...
   /***	bp->it_seg_prc = ...; ***/
	...
	switch (tp->tp_state)
	{
		...
		if ((bp->it_ctl & (C_ACK|C_SYN)) != C_SYN)
		...
		for
		(	lp = tp->tp_listen_q, end_listen_q = lp + LISTENQ_MAXSZ;
			/* abaixo */;
			lp++
		)
		{
	   ...
	   /***	lp->tp_listen_irs     = ...; /* acima ***/
		...
		if (bp->it_seg_len == 0)	/* Ou seja, somente um ACK */
		{
			...
			{
				if
				(	TCP_GE (seg_seq, rcv_nxt) &&
					TCP_LT (seg_seq, rcv_wnd)
				)
					OK++;
			}
		}
		...
		{
			if
			(	TCP_LT (bp->it_seg_ack, tp->tp_snd_una) ||
				TCP_GT (bp->it_seg_ack, tp->tp_snd_nxt)
			)
				goto drop_with_reset;
		...
		if (TCP_GT (bp->it_seg_ack, tp->tp_snd_nxt)) /* Ainda não enviado */
		{
			...
			{
				printf
				(	"%g: ACK não aceito (porta %d): "
					"seg_ack = %u, snd_nxt = %u, "
					"seg_wnd = %d\n",
					tp->tp_my_port,
					bp->it_seg_ack, tp->tp_snd_nxt,
					bp->it_seg_wnd - bp->it_seg_ack
				);
			}
#endif	MSG
			if (bp->it_seg_len > 0)
 ...
 */
void
do_eventdone_on_inqueue (register TCP_EP *tp)
{
#ifdef	MSG
 ...
 */
void
reset_the_connection (register TCP_EP *tp, int send_reset)
{
	/*
	 *	Envia o "RST"
	 */
