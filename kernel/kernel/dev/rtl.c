...
#define RL_RXCFG_CONFIG		(RL_RX_FIFOTHRESH|	\
				 RL_RXDMA_256BYTES|RL_RX_BUF_SZ)
...
typedef	struct
{
	...
	int		autoneg_cnt;	/* Auto-negociação */
	EVENT		autoneg_done;
...
void		rtl_eeprom_putbyte (int, int);
ushort		rtl_eeprom_getword (int, int);
void		rtl_read_eeprom (int, ushort *, int, int);
...
pci_rtl_attach (PCIDATA * pci, int unit)
{
	 ...
	 */
	rtl_read_eeprom (io_port, (ushort *)rp->ethaddr.addr, RL_EE_EADDR, 3);

	/*
	 *	Obtém o tipo
	 */
	rtl_read_eeprom (io_port, &devtype, RL_EE_PCI_DID, 1);
...
rtlopen (dev_t dev, int oflag)
{
	 ...
	 */
#if (0)	/*******************************************************/
	rxlen = BYUPPG (RL_RXBUFLEN + 32);

	rp->rxbuf = (char *)PGTOBY (malloce (M_CORE, rxlen));
#endif	/*******************************************************/

	rp->rxbuf = malloc_byte (RL_RXBUFLEN + 32);
...
#if (0)	/*******************************************************/
	txlen = BYUPPG (RL_TX_NSLOTS * (2 * KBSZ));

	if ((rp->txarea[0] = (char *)PGTOBY (malloce (M_CORE, txlen))) == NOSTR)
#endif	/*******************************************************/

	if (((rp->txarea[0] = malloc_byte (RL_TX_NSLOTS * (2 * KBSZ)))) == NOSTR)
	{
#if (0)	/*******************************************************/
		mrelease (M_CORE, BYUPPG (rp->rxbuf), rxlen);
#endif	/*******************************************************/
		free_byte (rp->rxbuf);
	...
	rp->rxbuf += 16;		/* Deixa uns bytes no início (?) */
...
rtlclose (dev_t dev, int flag)
{
	 ...
	 *	Libera os buffers
	 */
	free_byte (rp->rxbuf - 16);
	free_byte (rp->txarea[0]);

#if (0)	/*******************************************************/
	mrelease (M_CORE, BYUPPG (rp->rxbuf - 16), BYUPPG (RL_RXBUFLEN + 32));
	mrelease (M_CORE, BYUPPG (rp->txarea[0]),  BYUPPG (RL_TX_NSLOTS * (2 * KBSZ)));
#endif	/*******************************************************/
...
rtl_init (RTL *rp)
{
	int		s, i, port = rp->port;
	ulong		rxcfg = 0, rxfilt;
	ushort		phy_bmcr = 0;
	char		*cp;
	...
	/*
	 *	Configura transmissão e recepção
	 */
	...
	/*
	 *	Determina os pacotes a serem recebidos
	 */
	rxfilt = read_port_long (rp->port + RL_RXCFG);

	rxfilt |= RL_RXCFG_RX_MULTI;

	write_port_long (rxfilt,     rp->port + RL_RXCFG);
 ...
 */
#define EE_SET(x)					\
	write_port (read_port (port + RL_EECMD) | (x), port + RL_EECMD)

#define EE_CLR(x)					\
	write_port (read_port (port + RL_EECMD) & (~(x)), port + RL_EECMD)

void
rtl_eeprom_putbyte (int port, int addr)
{
	int		d, i;
 ...
 */
ushort
rtl_eeprom_getword (int port, int addr)
{
	int		i;
	ushort		word = 0;
	 ...
	 */
	write_port (RL_EEMODE_PROGRAM|RL_EE_SEL, port + RL_EECMD);

	/*
	 *	Envia o endereço a ser acessado.
	 */
	rtl_eeprom_putbyte (port, addr);
	...
	for (i = 0x8000; i != 0; i >>= 1)
	{
		...
		if (read_port (port + RL_EECMD) & RL_EE_DATAOUT)
			word |= i;
 ...
 */
void
rtl_read_eeprom (int port, ushort *addr, int off, int count)
{
	ushort		*endaddr;

	for (endaddr = addr + count; addr < endaddr; addr++, off++)
		*addr = rtl_eeprom_getword (port, off);
...
{
	RTL		*rp = &rtl[unit];
	ushort		phy_sts = 0, media, advert, ability;
	 ...
	 */
	phy_sts = rtl_phy_readreg (rp, PHY_BMSR);

	if ((phy_sts & PHY_BMSR_CANAUTONEG) == 0)
	{
	...
	if (rp->autoneg_cnt == 0)
	{
		...
		phy_sts  = rtl_phy_readreg (rp, PHY_BMCR);
		phy_sts |= PHY_BMCR_AUTONEGENBL|PHY_BMCR_AUTONEGRSTR;

		rtl_phy_writereg (rp, PHY_BMCR, phy_sts);
	...
	elif ((rtl_phy_readreg (rp, PHY_BMSR) & PHY_BMSR_AUTONEGCOMP) == 0)
	{
		/*
		 *	Após 1/2 segundo ainda não completou a negociação
		 */
	...
	phy_sts = rtl_phy_readreg (rp, PHY_BMSR);
	media   = rtl_phy_readreg (rp, PHY_BMCR);

	if ((rtl_phy_readreg (rp, PHY_BMSR) & PHY_BMSR_LINKSTAT) == 0)
		goto done;
	...
	rtl_phy_writereg (rp, PHY_BMCR, media);
