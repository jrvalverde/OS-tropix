...
t1_search_dir (IOREQ *iop, DIRENT *dep, int mission)
{
	...
	if (ip->i_sb->sb_mntent.me_flags & SB_ATIME)
		{ ip->i_atime = time; ip->i_flags |= ICHG; }
...
t1_write_dir (IOREQ *iop, const DIRENT *dep, const INODE *son_ip, int rename)
{
	...
	)
	{
	   ...
	   /***	dp->d_ent_sz	= ...;		/* Acima ***/
		...
		ip->i_mtime = time; ip->i_flags |= ICHG;
   ...
   /***	iop->io_rablock	    = ...;	***/
	...
	ip->i_nlink++; par_ino = ip->i_ino; /*** ip->i_flags |= ICHG; ***/ iput (ip);
	...
	ip->i_nlink--; ip->i_flags |= ICHG; iput (ip);
...
t1_make_dir (IOREQ *iop, DIRENT *dep, int mode)
{
	...
	pp->i_nlink++; pp->i_flags |= ICHG; 	/* Já levando em conta o ".." */
