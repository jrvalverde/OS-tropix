...
uhci_attach (PCIDATA *pci, ulong dev_vendor)
{
	 ...
	 */
	code = pci_read (pci, PCIR_COMMAND, /* bytes */ 1);

	if   (code & PCIM_CMD_PORTEN)
	{
		/* Mapeado em portas */

		base_addr = pci_read (pci, PCI_CBIO, 4) & ~3;

		sc->sc_bus.port_mapped	= 1;
		sc->sc_bus.base_addr	= base_addr;
	}
	elif (code & PCIM_CMD_MEMEN)
	{
		/* Mapeado em memória */
	   ...
	   /***	sc->sc_bus.port_mapped = 0; ***/
		...
		if ((sc->sc_bus.base_addr = mmu_map_phys_addr (base_addr, REGSZ)) == NULL)
		{
			printf ("uhci_attach: Não consegui mapear os registros de usb%d\n", usb_unit);
			goto bad;
		}
	}
	else
	{
		printf ("uhci_attach: usb%d NÃO está mapeado nem em portas nem memória\n", usb_unit);
		goto bad;
	}
...
uhci_open (struct usbd_pipe *pipe_ptr)
{
	...
	upipe->aborting = 0; upipe->nexttoggle = 0;
...
uhci_allocx (struct usbd_bus *bus)
{
	...
	if (xfer != NULL)
	{
		memclr (xfer, sizeof (struct uhci_xfer));

		((struct uhci_xfer *)xfer)->iinfo.sc = sc;
	}
...
uhci_waitintr (struct uhci_softc *sc, struct usbd_xfer *xfer)
{
	...
	for (/* acima */; timo >= 0; timo--)
	{
		...
		if (uhci_read_port_or_mem_short (sc, UHCI_STS) & UHCI_STS_USBINT)
		{
			uhci_intr1 (sc);
...
{
	struct uhci_softc	*sc = usb_data[frame.if_unit];
