...
ehci_attach (PCIDATA *pci, ulong dev_vendor)
{
	...
	printf ("ehci_attach: Controladora USB EHCI ainda NÃO suportada\n");
	...
	/*
	 *	Obtém o endereço base (sempre mapeado em memória)
	 */
	base_addr = pci_read (pci, PCI_CBMEM, 4) & ~3;

	if ((sc->sc_bus.base_addr = mmu_map_phys_addr (base_addr, 1024)) == NULL)
	{
		printf ("ehci_attach: Não consegui mapear os registros de usb%d\n", usb_unit);
		goto bad;
	}
...
ehci_init (struct ehci_softc *sc)
{
	...
	/*
	 *	Aloca o QH fictício que inicia a lista assíncrona
	 */
