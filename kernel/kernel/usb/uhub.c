...
#if (0)	/*******************************************************/
{
	...
	{
		printf ("%s: configuration failed, error = %s\n", dev->nameunit, usbd_errstr (err));
		goto bad;
	}
	...
	(	"%s: %d port%s with %d removable, %s powered\n",
		dev->nameunit, nports, nports != 1 ? "s" : "",
		nremov, udev->self_powered ? "self" : "bus"
	);
...
uhub_explore (struct usbd_device *dev)
{
	...
	for (port = 1; port <= hd->bNbrPorts; port++)
	{
...
#ifdef	USB_MSG
		printf ("uhub_explore: status change hub = %d port = %d\n", dev->address, port);
#endif	USB_MSG
		...
		if (up->device != NULL)
		{
			...
			printf
			(	"%s: dispositivo desconectado da porta %d\n",
				sc->sc_dev->nameunit, port
			);
		...
		{
			printf
			(	"uhub_explore (%s): dispositivo sem energia na porta %d %d\n",
				sc->sc_dev->nameunit, port
			);
		}
		...
		printf ("%s: dispositivo recém-conectado\n", sc->sc_dev->nameunit);
		...
		{
			printf
			(	"uhub_explore (%s): usbd_new_device failed, error = %s\n",
