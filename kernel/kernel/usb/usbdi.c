...
usbd_transfer (struct usbd_xfer *xfer)
{
...
#ifdef	USB_MSG
	printf
	(	"usbd_transfer: xfer = %P, flags = %d, pipe_ptr = %P, running = %d, dma_addr = %P\n",
		xfer, xfer->flags, pipe_ptr, pipe_ptr->running, xfer->dmabuf
	);
	 ...
	 */
	if ((err = pipe_ptr->methods->transfer (xfer)) && err != USBD_IN_PROGRESS)
	{
