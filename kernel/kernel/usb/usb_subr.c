...
usbd_get_string_desc (struct usbd_device *udev, int sindex, int langid, struct usb_string_descriptor *sdesc)
{
	...
	if (err = usbd_do_request (udev, &req, sdesc))
		return (err);
...
usbd_get_string (struct usbd_device *udev, int si, char *buf)
{
	...
	if (udev->langid == USBD_NOLANG)
	{
		...
		else
		{
			...
			udev->langid = UGETW (usp->bString[0]);
		}
	}
...
usbd_set_config_index (struct usbd_device *udev, int index, int msg)
{
	...
	if (err = usbd_get_desc (udev, UDESC_CONFIG, index, len, cdp))
		goto bad;
                ...
                int speed, int port, struct usbd_port *up)
{
	...
	for (hub = up->parent; hub != NULL && hub->speed != USB_SPEED_HIGH; hub = hub->myhub)
		/* vazio */;
	...
	up->device = udev;	dd = &udev->ddesc;

	/* Try a few times in case the device is slow (i.e. outside specs.) */

#ifdef	USB_MSG
	printf ("usbd_new_device: Vou tentar obter o descritor\n");
#endif	USB_MSG
	...
	for (i = 0; /* abaixo */; i++)
	{
		...
		if ((err = usbd_get_desc (udev, UDESC_DEVICE, 0, USB_MAX_IPACKET, dd)) == 0)
			break;

		usb_delay_ms (udev->bus, 200);
	}
	...
	if (err = usbd_set_address (udev, addr))
	{
		printf ("usb_new_device: set address %d failed\n", addr);
	...
	usb_delay_ms (udev->bus, USB_SET_ADDRESS_SETTLE);
...
usbd_reload_device_desc (struct usbd_device *udev)
{
	...
	if (err = usbd_get_device_desc (udev, &udev->ddesc))
		return (err);
