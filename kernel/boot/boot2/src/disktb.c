 ...
 */
entry ATA		*ide_ptr[NATAC*NATAT + 1]; /* Contém somente as unidades presentes */
entry int		ide_index;		/* Índice da entrada seguinte */
...
get_all_parttb (void)
{
	...
	/*
	 *	Gera a tabela do discos IDE
	 */
	for (app = ide_ptr; /* abaixo */; app++)
	{
		if ((ap = *app) == NOATA)
			break;
		...
		switch (ap->at_type)
		{
			...
			if (ap->at_bios_head)
			{
				...
				hdinfo.h_head  = ap->at_bios_head;
				hdinfo.h_sect  = ap->at_bios_sect;
				hdinfo.h_cyl   = ap->at_bios_cyl;
				hdinfo.h_flags = ap->at_flags;

				if (geo_tst_geometry (block0, &hdinfo) < 0)
				{
					printf
					(	"\e[34m"
			...
			{
				geo_get_parttb_geo (block0, &hdinfo, ap->at_disksz, ap->at_dev_nm);

				printf
				(	"%s: Usando a geometria (%d, %d) da tabela de partições\n",
					ap->at_dev_nm, hdinfo.h_head, hdinfo.h_sect
				);
			}

			parttb.pt_type   = 0;
			parttb.pt_size   = ap->at_disksz;
			parttb.pt_active = 0;
		    ...
		    case ATAPI_ZIP:
			if (bopen (dp) >= 0 && bread (dp, 0, block0, BLSZ) >= 0)
			{
				/* Analisa o tipo de ZIP */

				switch (ap->at_scsi->scsi_disksz)
				{
			...
			make_disktb_entry (dp, -1, -1, &parttb, 0, &hdinfo);
			make_zip_disktb_entry (dp, ap->at_scsi);
			break;

		    case ATAPI_CDROM:
			parttb.pt_type    = 0;
			parttb.pt_active  = 0;
			parttb.pt_size    = ap->at_scsi->scsi_disksz << (ap->at_scsi->scsi_blshift - BLSHIFT);
			hdinfo.h_bl_shift = ap->at_scsi->scsi_blshift;
	...
	for (spp = scsi_ptr; /* abaixo */; spp++)
	{
		...
		elif (sp->scsi_nopen)
		{
			...
			{
				geo_get_parttb_geo (block0, &hdinfo, sp->scsi_disksz, sp->scsi_dev_nm);
...
get_parttb (const DISKTB *dev_dp, const HDINFO *gp)
{
	 ...
	 */
	for
	(	reg_index = 0, reg_pp = (PARTTB *)(block0 + PARTB_OFF);
		reg_index < NPART;
		reg_index++, reg_pp++
	)
	{
