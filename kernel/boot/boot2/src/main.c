 ...
 ****************************************************************
 */
const char	pgversion[] =  "TROPIX boot2, Versão: 4.8.0, de 03.11.05\n";
...
main (void)
{
   ...
   /***	printf ("%s", pgversion); ***/		/* Em seg.32.s */
	...
	if (tst_value > 2 * GBSZ)
	{
		printf ("Memória de tamanho excessivo: truncando a 2 GB\n");
		tst_value = 2 * GBSZ;
	}
...
#undef	MEM_DEBUG
	...
	/*
	 *	Inicializa o controlador SCSI Adaptec 2940
	 */
	pci_init ();
 ...
 */
int
there_is_memory (long addr)
{
	ulong		*ptr;

	ptr = (ulong *)(addr - sizeof (ulong));
