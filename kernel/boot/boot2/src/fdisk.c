...
entry char		block0[BLSZ];	/* Para o bloco 0 do disco */
...
fdisk (void)
{
	...
	for (hpp = ide_ptr; /* abaixo */; hpp++)
	{
		...
		switch (hp->at_type)
		{
			...
			if (hp->at_scsi->scsi_disksz == ZIP_FLOPPY_CAPACITY)
				continue;
			break;

		}	/* end type */

		cmd[4] = hp->at_dev_nm[2];
#if (0)	/*******************************************************/
		cmd[4] = hp->at_letter;
#endif	/*******************************************************/
		goto found;
...
fdisk_get_cmd (char *cmd)
{
	...
	switch (*cp)
	{
		...
		for (EVER)
		{
			...
			if   (cp[0] == 'h')	/* IDE */
			{
...
#if (0)	/*******************************************************/
					if (cp[2] == hp->at_letter)
#endif	/*******************************************************/
					if (cp[2] == hp->at_dev_nm[2])
						break;
				}
				...
				switch (hp->at_type)
				{
					...
					printf
					(	"\nO dispositivo \"%s\" NÃO contém um tabela de partições\n",
						line
					);
					continue;
...
#if (0)	/*******************************************************/
				if (hp->at_type != ATA_HD && hp->at_type != ATAPI_ZIP)
				{
					printf
					(	"\nO dispositivo \"%s\" NÃO é um disco\n",
						line
			   ...
			   /***	gp = &generic_hdinfo; ***/
				...
				if (hp->at_bios_head)
				{
					gp->h_head  = hp->at_bios_head;
					gp->h_sect  = hp->at_bios_sect;
					gp->h_cyl   = hp->at_bios_cyl;
					gp->h_flags = hp->at_flags;
				}
				else
				{
					geo_get_parttb_geo (block0, gp, hp->at_disksz, hp->at_dev_nm);
				}

				fdisk_type   = IDE_TYPE;
				fdisk_data   = hp;
				fdisk_size   = hp->at_disksz;
				fdisk_disktb = dp;
				fdisk_hdinfo = gp;
			...
			elif (cp[0] == 's')	/* SCSI */
			{
			   ...
			   /***	gp = &generic_hdinfo; ***/
				...
				{
					geo_get_parttb_geo (block0, gp, sp->scsi_disksz, sp->scsi_dev_nm);
				}
	    ...
	    {
		PARTTB		*reg_pp;
		const ATA	*hp = fdisk_data;
		int		cyl_sz, reg_index;

		if (fdisk_type == IDE_TYPE && hp->at_cyl <= 1024)
		{
			printf
	   ...
	   /***	gp = &generic_hdinfo; ***/
		...
		memmove (block0 + PARTB_OFF, parttb, NPART * sizeof (PARTTB));

		*(ushort *)(block0 + MAGIC_OFF) = 0xAA55;
#ifdef	NO_WRITE
...
#endif	DEBUG
		bwrite (fdisk_disktb, 0, block0, BLSZ);
#endif	NO_WRITE
...
fdisk_read_parttb (const DISKTB *dev_dp)
{
   ...
   /***	memsetl (parttb, 0, sizeof (parttb) / sizeof (long)); ***/
	...
	if (*(ushort *)(block0 + MAGIC_OFF) != 0xAA55)
		printf ("\n*** O bloco 0 NÃO contém \"0x55AA\"\n");

	/*
	 *	Copia as partições regulares
	 */
	memmove (parttb, block0 + PARTB_OFF, sizeof (parttb));
